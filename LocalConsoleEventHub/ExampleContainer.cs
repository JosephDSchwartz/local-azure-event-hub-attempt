...
namespace LocalConsoleEventHub
{
    ...
    public sealed class TestAmqpBroker : IContainer
    {
        ...
        public TestAmqpBroker(IList<string> endpoints, string userInfo, string certValue, string[] queues)
        {
            ...
            for (int i = 0; i < endpoints.Count; i++)
            {
                ...
                else
                {
                    ...
                    this.listeners[i].SASL.EnableMechanism(new Symbol(MsSbCbsSaslProfile.Name), new MsSbCbsSaslProfile());
                }
            }
        }
        ...
        public void Start()
        {
            ...
            {
                System.Console.WriteLine($"Listener Address Information:\n\n" +
                                         $"Host: {listener.Address.Host}\n" +
                                         $"Port: {listener.Address.Port}\n" +
                                         $"Scheme: {listener.Address.Scheme}\n" +
                                         $"Username: {listener.Address.User}\n" +
                                         $"Password: {listener.Address.Password}\n" +
                                         $"Use SSL: {listener.Address.UseSsl}\n");
                listener.Open();
            }
        }
        ...
        sealed class BrokerMessage : Message
        {
            ...
            public BrokerMessage(ByteBuffer buffer) : base(Decode(buffer))
            {
                this.buffer = buffer;
        ...
        sealed class TestQueue
        {
            ...
            public void CreatePublisher(ListenerLink link)
            {
                ...
                Publisher publisher = new Publisher(link.Session, this, link, id);
                lock (this.syncRoot)
                {
            ...
            abstract class MessageResponder
            {
                private readonly string _name;
                private readonly Session _session;
                private uint _linkId;

                protected MessageResponder(string name, Session session)
                {
                    _name = name;
                    _session = session;
                    _linkId = 0;
                ...
                public void SendResponse(string replyTo)
                {
                    ...
                    var senderLink = new SenderLink(_session, _name + (++_linkId), replyTo);
                        //new Attach
                        //{
                    ...
                    {
                        Console.WriteLine($"An error occurred replying to {replyTo}. Exception details: {ex.Message}");
                    }
                    finally
                    {
                        senderLink.Close();
            ...
            sealed class CbsResponder : MessageResponder
            {
                public CbsResponder(Session session) : base("CbsResponder", session)
                {
                }

                protected override Message GetResponseMessage()
                {
                    return new Message
                    {
            ...
            sealed class Publisher
            {
                ...
                readonly ListenerLink receiverLink;
                readonly int id;
                private readonly CbsResponder _cbsResponder;

                public Publisher(Session session, TestQueue queue, ListenerLink link, int id)
                {
                    this.queue = queue;
                    this.receiverLink = link;
                    this.id = id;
                    this._cbsResponder = new CbsResponder(session);
                ...
                static void OnMessage(ListenerLink link, Message message, DeliveryState deliveryState, object state)
                {
                        ...
                        (errorCondition = (string)message.ApplicationProperties["errorcondition"]) != null)
                    {
                        ...
                        ((Message)message.Body).ApplicationProperties["operation"]?.ToString() == "put-token")
                    {
                        var messageBody = (Message)message.Body;
                        var replyTo = messageBody.Properties?.ReplyTo;

                        thisPtr._cbsResponder.SendResponse(replyTo);
                    }
                    else
                    {
                        ...
                        thisPtr.receiverLink.DisposeMessage(message, deliveryState, true);
                    }
                }
